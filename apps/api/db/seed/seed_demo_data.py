...
def get_seed_counts(
    ...
    """Return row counts for seeded demo tables."""
    ...
    return {
        ...
        "systems": session.scalar(
            select(func.count()).select_from(System)
        ) or 0,
        "assets": session.scalar(
            select(func.count()).select_from(Asset)
        ) or 0,
        "asset_health_snapshots": session.scalar(
        ...
        "document_chunks": session.scalar(
            select(func.count()).select_from(
                DocumentChunk
            )
        ) or 0,
        "document_asset_links": session.scalar(
            select(func.count()).select_from(
                DocumentAssetLink
            )
        ) or 0,
        "rca_cases": session.scalar(
            select(func.count()).select_from(RcaCase)
        ) or 0,
        "root_causes": session.scalar(
            select(func.count()).select_from(RootCause)
        ) or 0,
        "evidence": session.scalar(
            select(func.count()).select_from(Evidence)
        ) or 0,
        "maintenance_work_orders": session.scalar(
            select(func.count()).select_from(
                MaintenanceWorkOrder
            )
        ) or 0,
        "maintenance_events": session.scalar(
            select(func.count()).select_from(
                MaintenanceEvent
            )
        ) or 0,
        "compliance_rules": session.scalar(
            select(func.count()).select_from(
                ComplianceRule
            )
        ) or 0,
        "compliance_findings": session.scalar(
            select(func.count()).select_from(
                ComplianceFinding
            )
        ) or 0,
        "compliance_asset_summaries": session.scalar(
            select(func.count()).select_from(
                ComplianceAssetSummary
            )
        ) or 0,
        "dataset_snapshots": session.scalar(
            select(func.count()).select_from(
                DatasetSnapshot
            )
        ) or 0,
...
def seed_demo_database_from_url(
    database_url: str | None = None,
) -> dict[str, int]:
    """Seed PlantMind demo data using a SQLAlchemy database URL."""
    ...
    try:
        with SessionFactory() as session:
            ...
            return get_seed_counts(session)
    finally:
        engine.dispose()
...
def main(
    ...
    parser = argparse.ArgumentParser(
        description=(
            "Seed PlantMind demo data into PostgreSQL."
        )
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Reset existing demo rows before seeding. "
            "Required for safety."
        ),
    )
    ...
    parser.add_argument(
        ...
        action="store_true",
        help=(
            "Confirm destructive demo reset."
        ),
    )
    ...
    if not args.reset:
        raise SystemExit(
            "Refusing to seed without --reset. "
            "This command replaces demo rows."
        )

    if not args.yes:
        raise SystemExit(
            "Refusing to seed without --yes confirmation."
        )

    counts = seed_demo_database_from_url(
        database_url=args.database_url,
    )

    print("PlantMind demo data seeded successfully.")
